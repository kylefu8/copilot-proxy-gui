...
- Claude Code 集成：一键启动 & 写入/清除 `~/.claude/settings.json` 配置；启动时自动检测上次残留的代理配置并提示保留或清除
- 用量看板（配额进度条 + 详细数据折叠展示）
...
当前正式版本为 [v0.10.0-2](https://github.com/kylefu8/copilot-proxy-gui/releases/tag/v0.10.0-2)。`v0.10.0` 以来的累计多账号体验修复见 [RELEASE_NOTES.md](RELEASE_NOTES.md#v0100-2)，完整 `v0.10.0` 多账号升级说明见其后章节。
...
- Claude Code integration: one-click launch & write/clear `~/.claude/settings.json`; on startup, detects leftover proxy config from previous sessions and prompts to keep or remove
...
The current stable release is [v0.10.0-2](https://github.com/kylefu8/copilot-proxy-gui/releases/tag/v0.10.0-2). See [RELEASE_NOTES.md](RELEASE_NOTES.md#v0100-2) for the cumulative multi-account UX fixes since `v0.10.0`; the following section retains the complete `v0.10.0` upgrade notes.
